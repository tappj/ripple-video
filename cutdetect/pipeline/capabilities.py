...
MODEL_CAPABILITIES: dict[str, ModelCaps] = {
    "seedance2": ModelCaps(
        ...
        notes=(
            "The published Workflow runs at a 15-second maximum; Ripple preserves complete "
            "source sections and trims each result to its exact source-group duration."
        ),
    ),
    "seedance2_5": ModelCaps(
        ...
        notes=(
            "The published Workflow runs at a 15-second maximum; Ripple trims each result "
            "to its exact source-group duration."
        ),
    ),
    "hailuo3": ModelCaps(
        model_id="hailuo3",
        ...
        notes=(
            "The published Workflow accepts video, face, and voice references at 9:16 and "
            "Ripple trims each 15-second result to its exact source-group duration."
        ),
    ),
}
...
def credit_cost(
    ...
    *,
    ...
    if model_id == "seedance2_5":
        rate = 20 if ratio.lower() == "480p" else 30
        reference_rate = 10 if ratio.lower() == "480p" else 15
        return math.ceil(duration_s * rate + reference_video_duration_s * reference_rate)
    if model_id == "hailuo3":
