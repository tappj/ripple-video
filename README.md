...
Running is an explicit paid action. The direct Hailuo route gives every group its own
rounded output duration while sending the grouped source video, face image, voice sample,
and prompt on every task. Inspect `estimated_credits` from `prepare-job`, then use that
exact value as the initial ceiling. A higher ceiling is required if automatic retries
should be allowed:
